...
from src.imports import storage_client, logging
...
def read_file(file_path: str, is_binary: bool = False) -> bytes | str | Any:
...
def save_pdf_to_gcp(image: PILImage, output_path: str):
...
def fetch_image(image_path: str) -> Union[PILImage, None]:
    if image_path.startswith('http'):
        response = requests.get(image_path)
        logging.info(
            f"Fetched image from URL: {image_path}, Status Code: {response.status_code}, "
            f"Content Type: {response.headers['Content-Type']}, Content Length: {len(response.content)}")
        ...
        image_data = read_file(file_path=image_path, is_binary=True)
...
def write_file(file_path: str, content: bytes | str | list | io.BytesIO, is_binary: bool = False):
...
def read_content_files(base_path: str) -> dict:
...
def save_summaries_to_file(summaries: list[str], filename: str = 'summaries.txt'):
    ...
    try:
        ...
        write_file(file_path=full_path, content=summaries)
...
def determine_language(text: str, n: int = 100):  # n defaults to 100, adjust as needed
    try:
