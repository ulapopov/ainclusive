...
def load_font(font_name: str, size: int) -> ImageFont:
    try:
...
def get_gcp_credentials() -> service_account.Credentials:
    ...
    creds_json_file = os.environ.get("GCP_CREDENTIALS_FILE")
    if creds_json_file:
...
def clean_text(text: list | str) -> list[str] | str:
...
def fetch_text_content_from_gcs(
        bucket_name: str,
        file_path: str
) -> str:
...
def fetch_image_urls(
    ...
    if not base_path.endswith('images/'):
        base_path += 'images/'
    image_files = list_gcs_files(bucket_name=bucket_name, prefix=base_path)
    return [generate_signed_url(bucket_name=bucket_name, blob_name=file_path) for file_path in image_files]
