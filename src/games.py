from src.imports import logging, load_font
from PIL import Image as PILImage
from PIL import ImageDraw
from src.file_utils import ensure_directory_exists, fetch_image, write_file
...
def generate_matching_game(
    ...
    write_file(file_path=output_path, content=output.getvalue(), is_binary=True)
...
def generate_fill_in_the_blank(
    ...
    write_file(file_path=output_path, content=html_content, is_binary=False)

    logging.info(f"Successfully generated HTML for fill-in-the-blank, output path: {output_path}")


def generate_cut_and_paste(
        images: list[str],
        output_path: str
):
    ...
    for image_path in images:
        image = fetch_image(image_path=image_path)
        if not image:
            continue
...
def generate_labeling_activity(
        image_path: str,
        labels: list[str],
        output_path: str
):
    ...
    labeling_image = fetch_image(image_path=image_path)
...
def generate_sequencing_activity(
        images: list[str],
        output_path: str
):
...
def generate_games(
    ...
    ensure_directory_exists(directory_path=output_path)
    ...
    # Matching Game
    generate_matching_game(
        words=words,
        definitions=definitions,
        output_path=f"{output_path}matching_game.pdf"
    )
    ...
    generate_fill_in_the_blank(
        text=text,
        words=words,
        output_path=f"{output_path}fill_in_the_blank.html"
    )

    # Cut and Paste
    generate_cut_and_paste(
        images=images,
        output_path=f"{output_path}cut_and_paste.pdf"
    )

    # Table Completion
    generate_table_completion(
        headers=headers,
        output_path=f"{output_path}table_completion.pdf"
    )

    # Coloring Page
    generate_coloring_page(
        image_path=images[0],
        output_path=f"{output_path}coloring_page.pdf"
    )

    # Labeling Activity
    generate_labeling_activity(
        image_path=images[1],
        labels=labels,
        output_path=f"{output_path}labeling_activity.pdf"
    )

    # Sequencing Activity
    generate_sequencing_activity(
        images=images[:5],
        output_path=f"{output_path}sequencing_activity.pdf"
    )

    # Short Answer Questions
    generate_short_answer_questions(
        questions=questions,
        output_path=f"{output_path}short_answer_questions.pdf"
    )

    # True/False Questions
    generate_true_false_questions(
        statements=statements,
        output_path=f"{output_path}true_false_questions.pdf"
    )
    ...
    generate_multiple_choice_questions(
        questions=questions,
        choices=choices,
        output_path=f"{output_path}multiple_choice_questions.pdf"
    )
